...
static const char *TAG = "mqtt_meshko_lab";
...
static void send_button_event_json(void)
{
    ...
    cJSON_AddStringToObject(root, "user", "Meshko");
    cJSON_AddNumberToObject(root, "uptime", esp_log_timestamp());

    char *post_data = cJSON_PrintUnformatted(root);
    esp_mqtt_client_publish(mqtt_client, "/Meshko/button_event", post_data, 0, 1, 0);
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data) {
    ...
    switch ((esp_mqtt_event_id_t)event_id) {
        case MQTT_EVENT_CONNECTED:
            ESP_LOGI(TAG, "MQTT ПІДКЛЮЧЕНО (Meshko)");
            // Підписка на топік з прізвищем
            esp_mqtt_client_subscribe(mqtt_client, "/Meshko/led_control", 0);
            break;
